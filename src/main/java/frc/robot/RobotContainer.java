...
import frc.robot.subsystems.HangerArms;
...
public class RobotContainer {
  private static final double kLowerDeadband = 0.20;
  ...
  private final DoubleSupplier _hangerArmsInput = () -> filterControllerInputs( _operatorController.getLeftY());
  private final HangerArms _hangerArms = new HangerArms(_hangerArmsInput);
  ...
  private void configureButtonBindings() {
    var operatorYButton = new JoystickButton(_operatorController, XboxController.Button.kY.value);
    var operatorAButton = new JoystickButton(_operatorController, XboxController.Button.kA.value);
    ...
    var fireCommand = new InstantCommand(() -> {
      if(_limelight.isTargeting()){
        var power = _limelight.getShotPower();
        _shooter.setShooterPower(power[0]);
        _shooter.setKickerPower(power[1]);
      } else {
   ...
   rightTrigger.whenActive(reverseConveyorCommand.andThen(fireCommand))
    ...
    operatorYButton.whenPressed(new InstantCommand(() -> {
        _elevator.extend();
      }, _elevator), true)
      .whenReleased(new InstantCommand(()-> _elevator.stop(), _elevator), true);
    
    operatorAButton.whenPressed(new InstantCommand(() -> _elevator.retract(), _elevator), true)
      .whenReleased(new InstantCommand(()-> _elevator.stop(), _elevator), true);
    ...
    driverBButton.whenPressed(new InstantCommand(()-> {
      _limelight.setTargetPipeline();
      _drive.setRotationSupplier(() -> _limelight.trackTarget());
    }, _limelight), true)
    .whenReleased(new InstantCommand(() -> {
      _limelight.setCameraPipeline();
  ...
  private static double filterControllerInputs(double input) {
    ...
    //scale low deadband to upper deadband 0 to 1
    var range = kUpperDeadband - kLowerDeadband;
    var offset = Math.abs(input) - kLowerDeadband;
    var scaled = input > 0 ? offset / range : -offset / range;

    //cubing input so we have more control at lower values
    var filtered = scaled * scaled * scaled;
